...
if(setting.longestdesc!=""&&setting.descreveal!="none"){
fadeSlideShow.routines.adddescpanel($,setting)
if(setting.descreveal=="always"){
setting.$descpanel.css({top:setting.dimensions[1]-setting.panelheight})
setting.$descinner.click(function(e){
if(e.target.className=="close"){
slideshow.showhidedescpanel('hide')}
})
setting.$restorebutton.click(function(e){
slideshow.showhidedescpanel('show')
$(this).css({visibility:'hidden'})
})}
else if(setting.descreveal=="ondemand"){
...
setting.displaymode.type="manual"
...
clearTimeout(setting.hidedesctimer)
...
setting.$restorebutton.css({visibility:'visible'})
})
},
paginateinit:function($){
var slideshow=this
var setting=this.setting
if(setting.togglerid){
setting.$togglerdiv=$("#"+setting.togglerid)
setting.$prev=setting.$togglerdiv.find('.prev').data('action','prev')
setting.$next=setting.$togglerdiv.find('.next').data('action','next')
setting.$prev.add(setting.$next).click(function(e){
var $target=$(this)
...
.appendTo(setting.$wrapperdiv)
$('<div class="descpanelbg"></div><div class="descpanelfg"></div>')
.css({position:'absolute',left:0,top:0,width:setting.$descpanel.width()-8,padding:'4px'})
.eq(0).css({background:'white',opacity:0.7}).end()
