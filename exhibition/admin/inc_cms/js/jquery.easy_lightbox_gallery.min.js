(function($){var globals={settings:{},options:{},catlist:{},cache:{},current_category:"",isGalleryActive:false};$.fn.extend({putEasyGallery:function(options){var responseMessage="Error loading gallery , please check the CMS url.";if(!options)var options={};var defaults={url_server:"admin/gallery.php",nav_custom:"",nav_hor:"",nav_vert:"",catid:"",event_before_load:null,event_after_load:null,event_nav_click:null,event_get_catlist:null};var final_options={};if(globals.isGalleryActive==false){$.extend(final_options,defaults,options);globals.options=final_options;}else{final_options=globals.options;if(options.catid)
final_options.catid=options.catid;}
loadCssFile(final_options["url_server"]+"?action=getcss");this.each(function(){var deeplink_catid="";var t=this;$(document).ready(function(){if(final_options.catid!=""){if(globals.current_category!=""&&Number(final_options.catid)==Number(globals.current_category))
...
activateEvent("before_load");$.ajax({url:final_options["url_server"],global:false,type:"POST",data:({catid:final_options.catid,deeplink_catid:deeplink_catid,active:globals.isGalleryActive}),dataType:"html",async:true,success:function(response){activateEvent("after_load");var objResponse=eval("("+response+")");if(!objResponse.success||objResponse.success!=true){var message="";if(objResponse.message)
...
urlImage=generateImageUrl(final_options["url_server"],objItem.filename);var urlDownload=generateDownloadUrl(final_options["url_server"],objItem.filename);var desc=objItem.description;var title=objItem.title;if(desc==null)desc="";var lightboxText="";switch(globals.settings.view_text_fields){case"caption":lightboxText="<span class='lightbox_text_title'>"+title+"</span>";break;case"desc":lightboxText="<span class='lightbox_text_desc'>"+desc+"</span>";break;default:case"both":lightboxText="<span class='lightbox_text_title'>"+title+"</span>";if(desc.length>0)
lightboxText+="<span class='lightbox_text_sap'></span>";lightboxText+="<span class='lightbox_text_desc'>"+desc+"</span>";break;}
var html='';html+='<li title="'+title+'">';html+='<a href="'+urlImage+'" name="'+lightboxText+'">';if(globals.settings.thumb_text=="true"&&globals.settings.thumb_text_vert_pos=="top")
